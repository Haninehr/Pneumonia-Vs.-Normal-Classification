#Train :Sequentiel ::: Gabor => Fourier => DCT => Phog !
import cv2
import numpy as np
...
data_train_extracted="../Data/train_data_Sequentiel.csv"
data_val_extracted="../Data/val_data_Sequentiel.csv"
...
model = SGDClassifier(loss='log_loss', 
                    max_iter=1,
                    tol=None, #it defaults to 1e-4
                   
                    random_state=25, #or 42 !!
                    warm_start=True,
                    alpha=0.0001,#regularization strength. Regularization is used to prevent overfitting by adding a penalty to the loss function.
                    eta0=0.0001,#learning rate #0.00005 the best !
                    learning_rate='adaptive', #or 'constant" !
...
patience_for_loss=0
epochs_without_improvement_for_loss=0
best_val_accuracy = 0
...
for epoch in range(n_epochs):
    ...
    if val_accuracy > best_val_accuracy:
        ...
        epochs_without_improvement = 0
        joblib.dump(model, "../Models/ML_Sequentiel.pkl")  # Save the current best model
...
ax[0].plot(epochs , train_accuracies , 'go-' , label = 'Training Accuracy',markersize=1)
ax[0].plot(epochs , val_accuracies , 'ro-' , label = 'Validation Accuracy',markersize=1)
ax[0].set_title('Training & Validation Accuracy')
ax[0].legend()
ax[0].set_xlabel("Epochs")
ax[0].set_ylabel("Accuracy")

ax[1].plot(epochs , train_losses , 'g-o' , label = 'Training Loss',markersize=1)
ax[1].plot(epochs , val_losses , 'r-o' , label = 'Validation Loss',markersize=1)
ax[1].set_title('valing Accuracy & Loss')
ax[1].legend()
ax[1].set_xlabel("Epochs")
ax[1].set_ylabel("Training & Validation Loss")
plt.show()
model = joblib.load("../Models/ML_Sequentiel.pkl")
